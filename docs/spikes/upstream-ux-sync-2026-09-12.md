...
| One Plugins surface: Capabilities → Plugins owns agent plugins, desktop plugins, install and the catalog; `settings.nav.plugins` is gone from `en.ts` | `61afcde8f9` | Settings → Plugins, bundled-only, quoting a key that no longer exists — and quoting it under the wrong name, `settings.sectionEntries.plugins`, which has never existed at any pin this repo has held | drift | #227 |
