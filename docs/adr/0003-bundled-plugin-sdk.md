...
- The one machine datum a refusal carries is typed. `error.data.reason` — which the Gateway sends only for a `HostedRoomError` subclass that declares a `reason` (`tui_gateway/methods_groups.py:206-210`) — becomes `Refused.reason: PluginRefusalReason?`, a closed set of exactly the two reasons the backend defines (`room_history_expired`, `authority_conflict`; `gateway/hosted_rooms.py:185-198` @ `d177b119e9c56c9ddc0b7379ffce52341ec06584`). An unrecognized or missing string maps to null and the raw value never reaches a plugin, including through the result's `toString`; no branch keys off the numeric code, so a `4112` carrying no reason is a refusal this build cannot name rather than an assumed pruned room.
