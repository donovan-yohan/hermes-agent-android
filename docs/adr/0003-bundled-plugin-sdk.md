...
- Desktop's result-shaped refusal is kept. A Gateway `-32601` (`tui_gateway/server.py:734` @ `72a3277cd7937fd0f0a2a3e3fddbed21d7b1c8bd`, `unknown method: {method}`) maps to `PluginHostResult.UnavailableOnGateway` — the twin of `PluginRestResult.UnavailableOnGateway`; any other RPC error maps to `Refused` carrying this app's own sentence, never text the backend wrote; no live connection maps to `Refused(0, RECONNECT_MESSAGE)`, as `PluginRest` does with no transport.
- The one machine datum a refusal carries is typed. `error.data.reason` is what the hosted-room methods answer — `tui_gateway/methods_groups.py:206-210` @ `d177b119e9c56c9ddc0b7379ffce52341ec06584` maps a `HostedRoomError` through the method's `room_code` — and `Refused.reason: PluginRefusalReason?` is the closed set of exactly the two reasons those handlers declare (`room_history_expired`, `authority_conflict`; `gateway/hosted_rooms.py:185-198` @ the same SHA). `data.reason` is not hosted-room-only: `prompt.submit` answers `SESSION_NOT_OWNED` on the same member, which this build leaves unnamed (`data/gateway/GatewayRpc.kt`). An unrecognized or missing string maps to null and the raw value never reaches a plugin, including through the result's `toString`; no branch keys off the numeric code, so a `4112` carrying no reason is a refusal this build cannot name rather than an assumed pruned room.
...
- `host.onEvent(type, listener)` is Desktop's `onGatewayEvent` (`contrib/events.ts:16-28` @ `72a3277cd7937fd0f0a2a3e3fddbed21d7b1c8bd`): subscribe by type or `*`, listeners isolated so one throwing plugin cannot break the pump or another subscriber, disposer returned. Desktop fans every inbound event through `emitGatewayEvent` (`contrib/events.ts:31` @ `72a3277cd7937fd0f0a2a3e3fddbed21d7b1c8bd`) before its own dispatch; here the client's event stream is a broadcast — the bounded channel is the ingest buffer and one pump drains it to every subscriber, so the app's transcript pump and a plugin tap each receive every event rather than competing for one queue.
