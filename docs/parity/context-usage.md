...
| Number compacting logic and scale thresholds | `apps/shared/src/format.ts:1-26` |
| Gateway RPC method `session.context_breakdown`, and that it routes on `session_id` alone | `tui_gateway/methods_session.py:1193-1216`, `tui_gateway/server.py:1151-1167` |
| Gateway event `session.usage`, and that its ticker is joined before `message.complete` | `tui_gateway/server.py:3077-3100` (the ticker), `tui_gateway/prompt_turn.py:561-568` (its caller stops and joins it before anything emits) |
| `message.complete` carries the authoritative end-of-turn usage | `tui_gateway/prompt_turn.py:634-639,848` |
| Which colour the Gateway sends for a category, and that it filters `tokens > 0` | `agent/context_breakdown.py:18-27,167-172` |
...
| routing | Desktop sends `{ session_id }` alone (`use-context-breakdown.ts:41`) | Android also sends `profile`. It is defensive and unread: `_sess_nowait` resolves the session from `session_id` alone (`tui_gateway/server.py:1151-1167`), and this keeps the routing shape every other session RPC in `GatewaySessionRepository` sends |
...
| Category colours are CSS variables the browser resolves at paint (`apps/desktop/src/styles.css:217-224`) | The same eight expressions resolved once in the semantic token layer, `HermesTokens.contextUsage` | Android has no CSS custom properties. The Gateway sends only variable *names* (`context_breakdown.py:18-27`), so the names are resolved through the token group; a literal hex still parses and anything unrecognised falls back to `textTertiary`, the client's defensive default — the producer at the pin emits no unknown id |
...
| Desktop status item keeps its full connection label beside the meter; Android's phone row can run out of width | mobile-adaptation | When the meter and approval chip are both present, the connected subtitle uses the truthful compact label `Online`; no-meter and single-item rows retain their existing copy | At 320dp the meter and approval chip remain intact and the connection state remains legible rather than collapsing to an ellipsis; `ChatTopBarAlignmentTest` asserts the resolved label and geometry |
...
| All eight inks re-derived from `apps/desktop/src/styles.css:217-224` for every preset in both modes, and asserted distinct | `ThemeSemanticParityTest` |
...
| `session.info`, `session.usage` and `message.complete` all merge usage, absent keys keeping the last value, including provenance | `ContextBreakdownRepositoryTest` (`applyEvent` asserts streamed provenance, its preservation across a delta, and its replacement when `message.complete` carries it) |
