...
| The `Edit models…` row, its glyph, and that it closes the catalog | `apps/desktop/src/app/shell/model-catalog-menu.tsx:652-666` |
| That the picker honours the set, that search spans everything, and that the current model is pinned | `apps/desktop/src/app/shell/model-catalog-menu.tsx:715-770` |
| That the picker resolves the shortlist against the catalog it fetched before grouping | `apps/desktop/src/app/shell/model-catalog-menu.tsx:271-280` |
| That the picker groups the catalog **minus** the virtual `moa` provider, and where that row comes from | `apps/desktop/src/app/shell/model-catalog-menu.tsx:242-259`, `hermes_cli/inventory.py:736-743` |
| Every visible string | `apps/desktop/src/i18n/en.ts:3136-3141,3150` |
| That `model.options` carries no visibility field, and what `featured_models` is | `apps/desktop/src/types/hermes.ts:399-442`, `tui_gateway/methods_complete.py:469-490`, `hermes_cli/inventory.py:324-358` |
...
| curated default | `featured_models`, else the top 50 collapsed families (`:114-132`) | `expandProviderDefaults`, over `ModelProvider.featured` parsed from `model.options`. Both the picker and the Models sheet reach it the way Desktop does — through `effectiveVisibleKeys(stored, providers)`, resolved against the catalog actually fetched (`model-catalog-menu.tsx:274-280`) — so `groupModels`' top-N `else` branch has no counterpart here. The picker resolves over `pickerProviders` and the sheet over the unfiltered list, which is the same split Desktop makes (`:249-259` against `model-visibility-dialog.tsx:61-66`) |
...
| `Edit models…` is the last row of the whole catalog menu, after the MoA presets and the host footer (`model-catalog-menu.tsx:658-666`) | mobile-adaptation | Last row of the model list, above the adapted Reasoning and Fast sections | Desktop's reasoning and fast controls are per-row submenus, not sections; the phone sheet has them as sections below, so "the end of the list" is the row directly after the models |
| MoA preset rows above the footer (`model-catalog-menu.tsx:495-518`) | omission | Absent, and the virtual `moa` provider is filtered out before the shortlist resolves | deferred: #73 — Mixture-of-Agents presets are their own capability and no part of this batch. `pickerProviders` is Desktop's own filter (`:249-259`); without it the curated default would expand a preset name into the picker as though it were a model. The Models sheet keeps the row, exactly as Desktop's dialog does (`model-visibility-dialog.tsx:61-64`) |
| `Add provider…` closes the dialog and opens provider setup | omission | Visible and disabled behind the marker chip | coming soon — the pill already ships here (`ComingSoonAction`); provider setup is not ported, so the control is marked rather than missing. The marker was still pointing at #101 after the chip had landed |
...
| Provider rows are sorted alphabetically by name in the picker (`model-catalog-menu.tsx:766-768`) | omission | `model.options` order is kept | deferred: #73 — the picker's group order predates this change and is the same on both of its surfaces |
...
| `visibleModelGroups` over a resolved set is the curated default rather than a bare top-N, and it keeps a provider the stored set never named | `ModelVisibilityTest` |
| The picker drops the virtual `moa` row before resolving, while the Models sheet's own resolution keeps it | `ModelVisibilityTest` |
...
| `Edit models…` sits after the last model row and before the Reasoning section; the sheet carries Desktop's title, search and uppercase provider label; a switch hides one model and the reopened picker stops offering it; the provider checkbox bulk-toggles and reports its partial state; `Add provider…` ships disabled behind the marker chip | `ModelVisibilityJourneyTest` under Robolectric |
| The picker composable's own default is the host's `featured_models` shortlist rather than a bare top-N, a provider added after the last customisation still reaches it, and no MoA preset is ever offered there while the Models sheet still lists one | `ModelVisibilityJourneyTest` under Robolectric |
