...
| `sidebar.dateDivider` reads `Earlier today` / `Yesterday` / `Earlier this week` / `Last week` / `Earlier this month`, then a month name and month + year from `Intl` (`en.ts:2345-2351`; `lib/time.ts:125-165`) | drift | **Same five relative labels**, byte for byte (`SessionGrouping.kt:28-42`). Two deliberate exceptions: the first recents bucket takes the plain word when a Pinned section above it forced the label (`SessionGrouping.kt:45-67`, `:260-266`), and the tail is one `Older` bucket where Desktop emits one divider per calendar month, labelled from `Intl` (`lib/time.ts:155-165,30-31,169-190`) | #299. Desktop's strings are relational — `Earlier today` means *earlier than the head of this list*, which is only true while something newer sits above (`lib/time.ts:118-124`). Under a Pinned section the app's newest session can sit directly below the forced divider, so that slot keeps the plain `Today` / `This week` / `This month`; every earned divider keeps Desktop's copy, so `Earlier today` is still reachable exactly where it is truthful. The tail is not a word swap: a month name on one terminal bucket would caption rows belonging to other months, so it needs per-month divider identity (`m-<year>-<month>` / `my-<year>-<month>`) before the label can be true. The five strings are verified at `437116f9497c80d242ce034ff7f5d81dc277a337` (`en.ts:2794-2800`), the pin this work was read at |
