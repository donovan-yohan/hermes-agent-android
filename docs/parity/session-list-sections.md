...
| Row metadata | `['preview', 'updated']` by default: the preview line and the compact relative age (`store/layout.ts:308,318`) | The same two fields on every row, from the same bucket rule. The age's trailing slot and its spoken form are the two adaptations, ledgered below |
