...
| Two distinct captions: accent panel labels for `Pinned` / `Sessions` (`apps/desktop/src/app/shell/sidebar-label.tsx:11-19` @ `437116f9497c80d242ce034ff7f5d81dc277a337`), and quaternary date dividers with a trailing rule (`apps/desktop/src/app/chat/sidebar/chrome.tsx:135-140` @ `437116f9497c80d242ce034ff7f5d81dc277a337`) | mobile-adaptation | `PanelLabel` draws the section captions; `DateDividerLabel` draws the date buckets. The fixed drawer title also uses the panel treatment | #299. The additional accent caption belongs to fixed navigation chrome, not the scrolling list hierarchy. The two levels and the extra title are shown, rather than claimed pixel-identical, in `docs/parity/visual/session-list-months-captions/report.html` |
| `Sessions` heads the **unpinned pool** below `Pinned`; Desktop's `sessionsLabel` also names the project overview and entered project (`apps/desktop/src/app/chat/sidebar/index.tsx:1176-1181,1829` @ `437116f9497c80d242ce034ff7f5d81dc277a337`) | mobile-adaptation | `SessionListRow.SessionsLabel` heads the unpinned pool, while the drawer keeps its fixed pane title | #141. With pinned rows present, Android repeats the section name in two layers: fixed navigation chrome and a caption that scrolls with its rows. The distinction is visible in `docs/parity/visual/session-list-months-captions/report.html`; it is not a claim of identical header structure |
| Every row carries a right-aligned relative age from Desktop's default `['preview', 'updated']` metadata (`apps/desktop/src/store/layout.ts:300`; `apps/desktop/src/app/chat/sidebar/session-row.tsx:116-122,220-245`; `apps/desktop/src/i18n/en.ts:2340-2343` @ `3ca096de5f8183cb2e0ec23673f294d5978656a3`) | mobile-adaptation | The compact age remains visible beside the row action; its spoken form joins the row's merged accessibility sentence | #143. Touch has no hover, so the action reserves its own space rather than replacing the age. A screen reader visits one merged row instead of Desktop's separately focusable time element. `RelativeAgeTest` and `SessionListSectionsJourneyTest` cover formatting and semantics; rendered side-by-side evidence remains owned by #293 |
