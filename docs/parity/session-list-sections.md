...
| The five relative divider strings | `apps/desktop/src/i18n/en.ts:2794-2800` @ the same pin |
...
**Pending: #141 / #299.** There is no Android visual evidence for this change
yet, and none is claimed. The Android half of the exact-head side-by-side is
being captured by the exact-head workflow against the session-list-sections
surface; until that packet exists and its receipt validates, every statement on
this page about the panel captions, the `Sessions` caption inside recents, the
head-run cutoff and the per-month tail is an implementation claim from the
sources above, not a reading of pixels.

- pending: #141

Desktop does have a real render at the new pin — `hermes-agent` @ `437116f9497c80d242ce034ff7f5d81dc277a337`,
from a clean disposable export driven through the app's actual E2E mock backend
and real preload bridge, 14 synthetic rows (one pinned pair, one unread row, one
archived row), pinned clock `2026-09-17T14:20:00Z` in UTC, locale `en-US`, dark
theme. It is held as a provisional receipt: its manifest is not validated and
`docs/parity/visual-capture-surfaces.json` does not register the surface in this
change, so it is not cited here as acceptance.
