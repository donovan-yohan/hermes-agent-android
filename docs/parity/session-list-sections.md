...
| Desktop renderer and i18n, for the `#141`/`#299` caption, divider and date-group rows | `hermes-agent` @ `437116f9497c80d242ce034ff7f5d81dc277a337` | read-only checkout from a clean disposable export at that SHA; the cells carrying that pin were read with `git show <sha>:<path>` |

Every `path:line` below is against the first SHA unless its own cell carries
`@ 437116f9497c80d242ce034ff7f5d81dc277a337`. The `#141` / `#299` caption, divider and date-group rows were
re-read at that pin and are the only rows restamped; every other citation on
this page keeps the pin it was taken at.
...
| Row metadata | `['preview', 'updated']` by default: the preview line and the compact relative age (`store/layout.ts:300`) | The same two fields on every row, from the same bucket rule. The age's trailing slot and its spoken form are the two adaptations, ledgered below |
...
| The rendered sections: the `PINNED` label, the all-pinned sentence, the archived row's lead mark and spoken state, the archived empty state and the three states that are not it, the `Archived` row at a full touch target keeping the menu open, `Mark all as read` visible-and-disabled at zero, a working watermarked row still offering `Mark as read`, the default-on age on every row with its trailing placement, each verb reaching its caller with the row's id, and each verb handed off before the press returns | `app/src/testDebug/kotlin/com/hermesagent/mobile/ui/sessions/SessionListSectionsJourneyTest.kt` |
...
| `sidebar.dateDivider` reads `Earlier today` / `Yesterday` / `Earlier this week` / `Last week` / `Earlier this month`, then one divider per calendar month — a month name, then month + year from `Intl` (`apps/desktop/src/i18n/en.ts:2794-2800`; `apps/desktop/src/lib/time.ts:118-124,155-165`, formatters `:30-31` @ `437116f9497c80d242ce034ff7f5d81dc277a337`) | drift | **Same words, same tail, same head rule now.** `Earlier today` is reachable because the head is the newest *run*, cut at a real break exactly as Desktop cuts it: `headRunCutoff` at `data/session/SessionGrouping.kt:226-271` ports `apps/desktop/src/lib/session-date-groups.ts:44-88` (5 rows / 30 min / 8 h, `:266-272`), everything above the cutoff is unlabelled, and a bucket carrying the relational word therefore always has something newer above it. Past `Earlier this month` the tail is one divider per calendar month — key `m-<year>-<month>` inside the current year, `my-<year>-<month>` outside (`calendarBucket`, `:158`; the key keeps JavaScript's zero-based month, because the key is what a later read compares) — worded through ICU's own resolved skeleton rather than enumerated (`data/session/IcuSessionBucketLabel.kt:39,50-56`) | #299. What is left is not the wording or the bucketing: the five relative strings are Kotlin constants (`SessionGrouping.kt:49-59`), so a reader whose Desktop is translated still meets English here — that is this app's existing English-label scope, not new drift against this English issue. The head rule and the tail are now read off pixels at this head: the scrolled Android capture carries `EARLIER THIS MONTH`, then `JULY` and `NOVEMBER 2025` as two distinct per-month dividers, and the same words and the same cut sit in the Desktop reference (`docs/parity/visual/session-list-months-captions/`). The five strings are verified at the pin, and the per-month identity is what makes two month dividers able to coexist without either captioning the other's rows |
| Two distinct captions: `SidebarPanelLabel` for `Pinned` / `Sessions` — accent `--theme-primary` ink, tracking 0.16em, leading 8 px dither square (`apps/desktop/src/app/shell/sidebar-label.tsx:11-19`) — and `SidebarDateDivider` for the buckets — `--ui-text-quaternary`, tracking 0.12em, trailed by a hairline rule (`apps/desktop/src/app/chat/sidebar/chrome.tsx:134-140`; both @ `437116f9497c80d242ce034ff7f5d81dc277a337`) | mobile-adaptation | **Both treatments exist and are now spent as Desktop spends them**: `PanelLabel` — accent ink, leading dither square (`ui/common/HermesIcons.kt:362`), `type.panelLabel` (`ui/theme/HermesTypography.kt:65,139`) — on `PINNED`, `SESSIONS` and `Results` (`ui/common/Primitives.kt:113-133`; `ui/sessions/SessionList.kt:464,478,499`), and `DateDividerLabel` — quaternary ink, trailing rule (`ui/common/Primitives.kt:150-171`) — on every date bucket (`SessionList.kt:449`). One caption more wears the panel treatment: the drawer title (`SessionList.kt:216`) | The rail has no header row of its own to spend on Desktop's filter chrome, so the title row is the one caption that must name the pane and takes the accent treatment for it; inside the list the two-level hierarchy is Desktop's — accent section captions above quaternary time buckets. The title's placement is what `#141`'s exact-head comparison is for, and that comparison now exists: beside a pinned pool Android shows `PINNED` and `SESSIONS` as accent captions over date buckets wearing the quaternary ink and trailing rule, as Desktop spends them. The pane title is the one extra accent caption, and it is the drawer's fixed navigation chrome rather than a list row — recorded as an adaptation, not as pixel identity (`docs/parity/visual/session-list-months-captions/report.html`) |
| `Sessions` heads the **unpinned pool**, below `PINNED`, inside the list — the label Desktop's own `sessionsLabel` computes: `Sessions` flat, `Projects` at the overview, the entered project's name in project grouping (`apps/desktop/src/app/chat/sidebar/index.tsx:1176-1181`, rendered over the recents section at `:1829`; `apps/desktop/src/i18n/en.ts:2652` @ `437116f9497c80d242ce034ff7f5d81dc277a337`) | drift | `SessionListRow.SessionsLabel` now heads the unpinned pool below `PINNED`, in Desktop's own place and word (`data/session/SessionGrouping.kt:126-146,355`; rendered `ui/sessions/SessionList.kt:478`). The pane title above the list still reads `SESSIONS`, or the project label / `Projects` with the view (`SessionList.kt:189-196`) — the same word `sessionsLabel` gives the flat section | #141. With a pin above the pool the word is on screen twice: once as the pane title, once above the pool. Desktop has one because its header row carries the filter trigger while the section carries the caption. The exact-head comparison is where the duplication is judged, and this change keeps the word twice: the fixed pane title and the scrolling pool caption. They are not the same layer — the title belongs to the navigation chrome the drawer keeps while the list moves, the caption travels with the rows it names — so the duplication is recorded as a mobile-adaptation of Desktop's single header row, and the report shows both rather than calling the panes identical. If a later reading judges it wrong, this is the row where the title drops the pool's word (`docs/parity/visual/session-list-months-captions/report.html`) |
| The archived view keeps the `Pinned` section: it renders its own section over `pinnedSessions`, gated only on `!trimmedQuery` (`apps/desktop/src/app/chat/sidebar/index.tsx:1670-1674`), with the recents section starting separately at `:1677`; `showArchived` swaps the pool it reads and the recents grouping, never the pinned section (`:379,517,1348-1351,1736` @ `437116f9497c80d242ce034ff7f5d81dc277a337`). The committed Desktop contract's node text is `PINNED Refactor auth middleware 26m SESSIONS Onboarding copy pass 2d` | drift | A flat list with no caption: `buildSessionRows` short-circuits at `if (archivedView) return localMatches.map(SessionListRow::Row)` (`data/session/SessionGrouping.kt:331`), before the pinned split and the captions it carries (`:345,355`) | #146. A reader who pins a chat and then archives it loses the pin's only visible effect. The `archivedView` KDoc read `:1723` as authority for "no pinned section and no dividers"; `grouping='none'` settles the recents dividers only, and the comment is corrected in this change |
| Every row carries a right-aligned relative age — `12m`, `9h`, `1d`, `39d` — from the default row meta `['preview','updated']` (`store/layout.ts:300`; rendered at `session-row.tsx:116-122,220-245`; units `en.ts:2340-2343`) | mobile-adaptation | **Same** compact age in every row's trailing slot, always visible, at `--ui-text-tertiary` | The age is the same field Desktop's default turns on, from the same one bucket rule (`coarseElapsed`, `lib/time.ts:199-215`) and the same four units. Two mechanics changed. Desktop swaps its trailing figures out for the kebab on hover (`session-row.tsx:109,292-330`); touch has no hover, so the row reserves the control's width instead and the age stays put. And Desktop's `<time>` is focusable with its own `aria-label={`${age}, ${absoluteAge}`}` (`:229-237`); here the row is one merged semantics node, so the age joins that sentence in words (`Updated 12 minutes ago`) rather than becoming a second thing to visit — the compact `12m` reads as a number and a letter aloud |
...
- commit: 9aa0ae80687a65a11ea84279fd75cf46f9619ae3
...
| Android | `1457d75eb8e7e8336efa45c68fd2da5f5c34da70` — its code tree `2c655a09b15debbfd5dfa5e45b9eef29cc30c6cd` is the tree committed HEAD carries, so the capture and the page differ in `docs/parity` only — debug APK `99b356601d…`, local and installed SHA-256 equal, on the pinned emulator shape |
...
- **The two panes are not pixel-identical.** `SESSIONS` stays at the top of the
  Android pane and again above the scrolling pool. The title is the drawer's
  fixed navigation chrome and does not scroll away with the list, so the
  duplication is Desktop's single header row adapted to a drawer — a
  mobile-adaptation, left on the page as a discrepancy rather than smoothed
  into a claim of identity (see the caption-hierarchy row above).
