...
| `sidebar.dateDivider` reads `Earlier today` / `Yesterday` / `Earlier this week` / `Last week` / `Earlier this month`, then a month name and month + year from `Intl` (`en.ts:2794-2800`; `lib/time.ts:125-165`) | drift | **Same five relative labels**, byte for byte (`SessionGrouping.kt:29-42`). The tail is the remaining difference: one `Older` bucket where Desktop emits one divider per calendar month, labelled from `Intl` (`lib/time.ts:155-165,30-31,169-190`) | #299. The copy half closed in #141 — the `Earlier …` prefix was ported, and it is truthful here because this list already leaves its first group unlabelled (`SessionGrouping.kt:206-221`), the same rule that makes the word true upstream (`lib/time.ts:118-124`). The tail is not a word swap: a month name on one terminal bucket would caption rows belonging to other months, so it needs per-month divider identity (`m-<year>-<month>` / `my-<year>-<month>`) before the label can be true |
