...
| The archived view keeps the `Pinned` section: it is a section of its own gated only on `!trimmedQuery` (`sidebar/index.tsx:1640-1661`), and `showArchived` changes only the recents grouping (`:1723`, `grouping='none'`). The committed Desktop contract's node text is `PINNED Refactor auth middleware 26m SESSIONS Onboarding copy pass 2d` | drift | A flat list with no caption: `buildSessionRows` short-circuits at `if (archivedView) return localMatches.map(SessionListRow::Row)` (`data/session/SessionGrouping.kt:238`), before the pinned split at `:246-254` | #146. A reader who pins a chat and then archives it loses the pin's only visible effect. The `archivedView` KDoc read `:1723` as authority for "no pinned section and no dividers"; `grouping='none'` settles the dividers only, and the comment is corrected in this change |
