...
| Desktop renderer, for the citations re-resolved by [#146](https://github.com/donovan-yohan/hermes-agent-android/issues/146) | `hermes-agent` @ `437116f9497c80d242ce034ff7f5d81dc277a337` | same read-only checkout; the rows that carry the pin inline were taken at that SHA, where the `Pinned` and archived-section block sits twelve lines lower |

Every `path:line` below is against the first SHA unless the citation names the second one.
...
| Empty recents while everything is pinned | `Everything here is pinned. Unpin a chat to show it in recents.` (`en.ts:2660` @ `437116f9`), chosen by `pinnedSessions.length > 0 ? s.allPinned : s.noSessions` (`sidebar/index.tsx:1710-1712` @ `437116f9`, no `showArchived` term) | **Same**, verbatim, as one tertiary line below the section — in the live list and in the Archived view alike, because Desktop picks it from the same expression in both |
...
| The rendered sections: the `PINNED` label above live and archived rows alike, the all-pinned sentence, the archived row's lead mark and spoken state, the archived empty state and the three states that are not it, the `Archived` row at a full touch target keeping the menu open, `Mark all as read` visible-and-disabled at zero, a working watermarked row still offering `Mark as read`, the default-on age on every row with its trailing placement, the pinned row rendered once above the archived rows with no dividers beneath, each verb reaching its caller with the row's id, and each verb handed off before the press returns | `app/src/testDebug/kotlin/com/hermesagent/mobile/ui/sessions/SessionListSectionsJourneyTest.kt` |
...
| `sidebar.dateDivider` reads `Earlier today` / `Yesterday` / `Earlier this week` / `Last week` / `Earlier this month`, then a month name and month + year from `Intl` (`en.ts:2345-2351`; `lib/time.ts:125-165`) | drift | **Four of the five** relative labels are Desktop's, byte for byte (`SessionGrouping.kt:28-42`): `Yesterday`, `Earlier this week`, `Last week`, `Earlier this month`. Three deliberate differences: `Earlier today` is not rendered at all, the first recents bucket takes the plain word when a Pinned section above it forced the label (`SessionGrouping.kt:73-76`, `:312-315`), and the tail is one `Older` bucket where Desktop emits one divider per calendar month, labelled from `Intl` (`lib/time.ts:155-165,30-31,169-190`) | #299. Desktop's strings are relational — `Earlier today` means *earlier than the head of this list*, true only while something newer sits above (`lib/time.ts:118-124`). Under a Pinned section the app's newest session can sit directly below the forced divider, so that slot keeps the plain `Today` / `This week` / `This month`. And `Earlier today` is unreachable here even without pins: `calendarBucket` returns `Today` for every `dayDiff <= 0` (`SessionGrouping.kt:173`), the list sorts newest-first (`:257`), and buckets are contiguous and monotonic — so `Today` is only ever the **first** recents bucket, unlabelled without pins. Desktop reaches the word only by splitting *within* a day at a head-run cutoff (`session-date-groups.ts`, `headRunCutoffMs`), a heuristic this slice does not port (`SessionGrouping.kt:11-14`). The word is kept out rather than rendered as a claim the list cannot support. The tail is likewise not a word swap: a month name on one terminal bucket would caption rows belonging to other months, so it needs per-month divider identity (`m-<year>-<month>` / `my-<year>-<month>`) first. The five strings are verified at `437116f9497c80d242ce034ff7f5d81dc277a337` (`apps/desktop/src/i18n/en.ts:2794-2800`), the pin this work was read at |
| Two distinct captions: `SidebarPanelLabel` for `Pinned` / `Sessions` — accent `--theme-primary` ink, tracking 0.16em, leading 8 px dither square (`app/shell/sidebar-label.tsx:9-22`) — and `SidebarDateDivider` for the buckets — `--ui-text-quaternary`, tracking 0.12em, trailed by a hairline rule (`sidebar/chrome.tsx:51-97`) | drift | The panel-label treatment **exists here** — `DitherMark` (`ui/common/HermesIcons.kt:293-295`) plus `type.panelLabel` (`ui/theme/HermesTypography.kt:94`) in accent ink — but it is spent on the drawer title (`ui/sessions/SessionList.kt:151,191`). `PINNED` and the date buckets both fall to one `SectionLabel`: `textTertiary`, no glyph, no rule (`ui/common/Primitives.kt:87-94`) | #141. The drift is assignment, not a missing token: `PINNED` reads as another date bucket, and Desktop's two-level hierarchy flattens to one. Visible in `docs/parity/visual/session-list-sections/` |
| `Sessions` heads the **unpinned pool**, below `PINNED`, inside the list (`sidebar/index.tsx:1663-1666`, and the archived contract's `PINNED … SESSIONS …` node text) | drift | `SESSIONS` is the panel title above everything, including `PINNED`, and swaps to the project label or `Projects` with the view (`SessionList.kt:151,189-196`) | #141. Same caption, different job: Desktop's names one section among several, Android's names the pane. The recents pool therefore has no caption at all here, and the reader is given no boundary between the pinned rows and the rest apart from the first date divider |
| An archived all-pinned pool renders `No sessions match these filters` (`apps/desktop/src/i18n/en.ts:2666` @ `437116f9`): the recents empty state tests `filtersActive` before `allPinned` (`sidebar/index.tsx:1706-1712` @ `437116f9`), and `$sidebarFiltersActive` counts `$sidebarShowArchived` itself (`store/layout.ts:392-396` @ `437116f9`), so `s.allPinned` is unreachable while the toggle is on | mobile-adaptation | The archived view's empty recents keeps `Everything here is pinned. Unpin a chat to show it in recents.` (`SessionGrouping.kt:322`) — this app's own sentence for that slot, the same one the live list shows | Desktop's `these filters` names a `Status` / `Project` / `Profile` / `PR` filter surface this rail does not have (six controls omitted, owed to #142 above), so the sentence would point the reader at a control that does not exist. The note names the actual reason the archived recents area is empty — every archived row is pinned — which is the only one reachable here. Rendered by `SessionListSectionsJourneyTest`, ordered by `SessionGroupingTest` |
...
on a Pixel 10 Pro emulator in light theme against a `kame-qa` QA profile seeded
with four synthetic sessions among its existing QA rows — one pinned pair, one
unread row and one archived row. Three states, both sides each:
...
mark in the same slot. **#66 stays open.** Six filter-menu controls render on Desktop and are absent
...
**The archived-view half of this packet predates #146's fix.** That capture is
what *caught* the drift — Desktop's `PINNED … SESSIONS …` beside Android's flat
list — so it is evidence for the finding, not for the current render. Android
now draws the `PINNED` caption above the archived rows exactly as Desktop's
packet shows, and the stored packet still shows the pre-fix flat list. A fresh
side-by-side for that state is owed:
[#293](https://github.com/donovan-yohan/hermes-agent-android/issues/293), whose
session-list surface renders `SessionList` from a pinned clock with a pinned row
and an archived row — the same two-row state this needs. What stands in for it
today: the layout, ordering, single-rendering, no-divider and all-pinned
assertions in `SessionListSectionsJourneyTest`, and the row-order assertions in
`SessionGroupingTest`.

The row age (#143) landed after that capture, so the committed packet predates
it: `docs/parity/visual/session-list-sections/android/reference.png` shows rows
without the trailing age and is not evidence for the field. The capture surface
that produced it has no session-list state — `docs/parity/visual-capture-surfaces.json`
catalogues composer surfaces only — and this environment has no attached device
or emulator, so the age's own side-by-side is still owed:
[#293](https://github.com/donovan-yohan/hermes-agent-android/issues/293), which
adds the session-list surface and captures it from a pinned clock across the
`now` / `m` / `h` / `d` buckets. What stands in for it today: the pinned-clock
layout and semantics assertions in `SessionListSectionsJourneyTest` and
`RelativeAgeTest`, and Desktop's own sources at the pin.
