...
This is the **read-only** first slice of #191: a bot-scoped Routines
destination that issues one
`cron.manage {action:"list", include_disabled:true, profile:<bot>}` and renders
the result. Creation, pause/resume, delete, the schedule picker, delivery
targets, continuity and the legacy auto-pause sweep are #191's remaining scope
and are **not** implemented; the controls Desktop has for them ship here
visible, disabled and marked `WIP`.
...
| The row's controls | `cron.tsx:539-576`: a title button, a pause/resume Switch and a delete control, as siblings | `BotRowItem` keeps the same split — the row's tap and the Routines control are siblings — and the pause/resume and delete controls are marked disabled |
| Read-only by construction | — | No `add`, `remove`, `pause` or `resume` is sent anywhere in this slice: `BotsRoutinesRepositoryTest`'s `no mutation of any kind is issued by a read` asserts the only `action` that leaves is `list` |
...
control. Two sentences are this app's own because Desktop has no counterpart:
...
| The header's New-cron control, and the row's pause/resume Switch and delete control (`cron.tsx:1270-1274`, `:559-575`) | omission | All three render visible and **disabled** behind the `WIP` marker chip; none can issue a mutation | coming soon — they are #191's remaining scope, and an omitted control would say the surface was never meant to have one |
...
`populated` and a `read-failure` state, rendered by the debug-only
