...
| Pending state | `connection-switcher.tsx:133,272` (`aria-busy`, spinner) | `stateDescription = "Connecting…"` on the trigger and a per-row `Connecting…`; the registry row's switch action reads `Connecting…` from the same `ConnectionsCopy.CONNECTING` constant |
...
- `ConnectionsJourneyTest`, `ConnectionSwitcherJourneyTest` — the rendered
  ...
  `Connecting…` and neither it nor any sibling will take a second tap; `Test`
  ...
  pane's own `Connect` on screen above the list.
