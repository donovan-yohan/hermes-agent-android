...
| Hidden for one source | `connection-switcher.tsx:123-125` | `ConnectionsUiState.switchable`; the rail renders no chrome |
...
| Switch semantics | `store/connections.ts:153-225` (`selectConnection`) and `store/gateway-switch.ts:48-97` (`wipeSessionListsForGatewaySwitch`) | `data/connections/ConnectionSwitchController.kt` plus `SessionCache.resetForEndpointSwitch()` |
...
| `ConnectionSwitcher` is mounted once, in the statusbar (`app/shell/hooks/use-statusbar-items.tsx:433,664-668` @ `72a3277cd7`); the Gateways page has no connection chooser of its own | The same composable, mounted a second time at the top of the Gateways pane, above the **Connection mode** cards | **Adaptation**, approved by the owner on 2026-08-30. Desktop's statusbar is on screen beside the settings window, so switching is always one click away from the page where gateways are managed; a phone's Gateways screen is a destination, and the person who has just added or repaired a gateway would otherwise have to leave it to start using what they saved. It is the same composable rather than a second one — a forked trigger is how two switchers end up disagreeing about which gateway you are on — so the trigger anatomy, the sheet, the row order, the checkmark, the eight-connection search threshold and the `selectConnection` path are Desktop's, unchanged. Desktop's hide-below-two rule (`connection-switcher.tsx:123-125`) is kept here too, and costs nothing: with one saved row this pane is already showing that row's route, its fields and its `Current` pill. |
| The trigger's accessible name is the registry's own section title — `title={t.settings.connections.title}` rendered as `` `${title}: ${label}` `` (`connection-switcher.tsx:139,159,266` @ `72a3277cd7`) | Unchanged in the session rail's mount; the Gateways mount is named "Switch gateways" instead | **Adaptation**, and only on the one screen that needs it. Desktop can reuse the section title because the switcher is in the statusbar and the registry is a settings page — a screen reader never meets the string twice. This app puts both on the Gateways screen, where the same words would be announced as this button and again as the heading over the list a few rows below. The replacement is Desktop's own verb for the act: `stagedNote` is "Switch gateways from Sessions" (`en.ts:706`). Desktop already parameterises this as a prop (`:248`), so the composable takes a `title` whose default is the value Desktop passes — the rail keeps Desktop's label without saying anything. |
| The statusbar is persistent chrome, so its switcher is reachable from anywhere in the window without scrolling (`app/shell/hooks/use-statusbar-items.tsx:431`) | The Gateways mount scrolls with the page it sits at the top of | **Adaptation**, low. It is the first thing in the pane's scroll, so it is on screen when the screen opens, but scrolling down to the registry takes it away — Desktop's never leaves. Pinning it would cost the route's own form the height that #100 already refused to spend on a pinned mode grid, for a control the person uses once per visit. The session rail's mount is this app's always-reachable one, exactly as Desktop's statusbar is. |
| The trailing `DropdownMenuItem` navigates to the settings connections tab, and its `onConnect` prop is required (`connection-switcher.tsx:40,234-236`) | Rendered in the session rail's mount; not rendered in the Gateways mount, and the hairline above it goes with it | **Adaptation**, forced by the mount above: on Android that destination *is* the Gateways screen, so from this screen the item would be a door back into the room you are standing in. The composable takes a nullable `onManage` rather than being duplicated, so both mounts stay one control and a mount that has somewhere to go still renders Desktop's item, last, below its hairline. Adding a gateway from this screen is unchanged and still Desktop's one add path — the registry's `Add connection` button below (`connections-registry.tsx:819-829`, `en.ts:716`). |
...
| `cloud` kind | Rendered in the kind chooser, in Desktop's position, disabled behind the `WIP` pill (`ConnectionsSection.kt:635` — `ConnectionKindChoice(null, …)`) | **Coming soon**, not a non-goal. `ConnectionKind` deliberately has no member for it, so no saved row can be `cloud` and the choice cannot be taken — but the chooser still teaches the four kinds Desktop's does. A hosted instance is reachable today through the Remote kind; see the divergence row below and [Hermes Cloud](../guides/getting-started.md#hermes-cloud). Tracked by [#151](https://github.com/donovan-yohan/hermes-agent-android/issues/151). |
...
| `ConnectionSwitcher` mounted in the statusbar only (`app/shell/hooks/use-statusbar-items.tsx:433,664-668`) | mobile-adaptation | The same composable mounted a second time at the top of the Gateways pane, above the mode cards | Owner-approved 2026-08-30. Desktop's statusbar sits beside the settings window; a phone's Gateways screen is a destination you navigate to, so without this the person who just saved a gateway must leave the screen to use it. One composable, two mounts — trigger, sheet, order, checkmark, search threshold and select path unchanged. Desktop's hide-below-two rule (`connection-switcher.tsx:123-125`) applies on this pane as well |
| Trigger's accessible name is the registry's section title, `` `${title}: ${label}` `` (`connection-switcher.tsx:154,248,264`) | mobile-adaptation | Unchanged in the rail's mount; the Gateways mount is named "Switch gateways" | Desktop's two surfaces never meet, so reusing the section title costs nothing there; this app renders the switcher and that heading on one screen, where a screen reader would announce the same words as a button and as a heading. The replacement is Desktop's own verb for the act (`stagedNote`, `en.ts:706`), and Desktop already makes the label a prop, so the rail keeps Desktop's value as the default |
| The statusbar is persistent chrome; its switcher never scrolls away (`app/shell/hooks/use-statusbar-items.tsx:431`) | mobile-adaptation | The Gateways mount is the first thing in the pane's scroll and scrolls with it | On screen when the screen opens, gone once you scroll to the registry. Pinning it would spend on a once-per-visit control the height #100 already declined to give a pinned mode grid; the session rail's mount is this app's always-reachable one |
| The trailing `Manage gateways…` item navigates to the settings connections tab; the `onConnect` prop is required (`connection-switcher.tsx:40,234-236`) | mobile-adaptation | Rendered in the rail's mount, suppressed in the Gateways mount along with its hairline | That destination is this screen, so from here the item would lead back to itself. A nullable `onManage` parameterises the one composable instead of forking it, so the mount that has somewhere to go still renders Desktop's item last, below its hairline |
...
| A failed switch is a toast raised by the click handler — `selectConnection` throws "did not become active" and `choose` calls `notifyError(…, switchConnectionFailed(label))` (`store/connections.ts:198-200`, `connection-switcher.tsx:123-128`, `en.ts:1770`) | mobile-adaptation | The same sentence, verbatim, as an inline line under the switcher trigger at both mounts, with a `Dismiss` target beside it | This app has no in-app notification stack to raise a toast into — epic #73 lists one as unbuilt — so the report is anchored to the control that started the switch instead of floating over the app. It is a line rather than a toast, so it needs an end: it goes when the person dismisses it, when the next switch starts, and when a gateway does come up. It carries `LiveRegionMode.Polite`, because Desktop's toast announces itself by being a toast and a line that simply appears announces nothing — same politeness `PendingInputSurface` uses for the other thing that arrives unasked. What is still lost against Desktop is the toast's own transience and its notification-region grouping. Dismissal is keyed on the report rather than the row, so dismissing one failure cannot silence the next, but it is held per mount in `rememberSaveable` rather than by the one writer: `ConnectionsActions` is assembled in `MainActivity`, which this change may not touch — a deferral to #116 S-U5..S-U7, which owns that file (#116 S-U1) |
| Two outcomes Desktop does not toast: a row nothing was going to dial, and one already active | mobile-adaptation | `ConnectionSwitchOutcome.Unattended` and `Ignored` are not reported | Desktop's registry cannot switch and its Managed-SSH equivalent is main-process-owned, so neither case exists there. Here the unattended row already carries `SSH_NEEDS_CREDENTIAL` / `NEEDS_CONNECT` naming its own next action, and a second sentence would be the same fact twice |
| Registry rows carry no connection status at all: `Current`, `Primary` and `App-managed` are registry facts, and whether the device is up is the statusbar's business (`connections-registry.tsx:578-641`, `en.ts:713-715`) | mobile-adaptation | The current row's leading action slot becomes `Sign in` (Remote, `en.ts:818`) or `Connect` (Local, `en.ts:18`, `common.connect`) while this device is asking for one, and presses the route pane's own Connect | Forced by the row above about switching from this list: because this list switches, the row *is* the thing that just failed, and a row reading only `Current` about a gateway that never came up is a false claim (#116 S-U2). It occupies the slot `Switch` vacates on the active row, so the cluster's one-line measurement is unchanged (`ConnectionRowActionsLayoutTest`). Managed SSH is deliberately given no such control: its credential is typed into the form above and lives in memory, so the button would dial with nothing |
| A gateway that cannot be reached takes over the window as a failure view with a `Gateway settings` action (`en.ts:87-96`) | mobile-adaptation | No takeover; the chat chrome's own connection line becomes the action, in the header subtitle and on the composer where it is rendered | A phone has one surface at a time, and a full-screen takeover would put the transcript behind a door. The line was already saying what to do and could not be tapped, which cost three taps and a scroll through Settings to reach the sign-in (#116 S-U3). `Connecting` is deliberately not a door — it reports what is already happening — and the destination is the one `Manage gateways…` reaches, which is also the line's spoken name |
| The pending badge is held for the whole dial (`connection-switcher.tsx:133,272`) | mobile-adaptation | A Remote row whose stored sign-in is missing or bound to another Gateway is not waited on at all | Desktop's dial can always present a credential — its sign-in lives in the main process. Here a slot that reads empty or refused fails with a 401 this app deliberately never retries (`GatewayConnection.isRetryableRemoteConnectionFailure`), so the settle window could only end one way while holding a `Connecting…` badge, and the switch lock, for up to twenty seconds (#116 S-U4) |
...
| `Update all instances` (`connections-registry.tsx:969-987`, `en.ts:786`) | omission | Beside `Add connection`, in Desktop's own `connections.length > 1` gate, disabled behind the `WIP` chip | coming soon — the pill ships in this change (#101). There is no multi-instance update path here: the System panel updates the one backend this app is connected to, and a fan-out needs a registry the main process owns. `ConnectionEditorLayoutTest`, `ConnectionsJourneyTest` |
| Launch-mode `ToggleRow` (`connections-registry.tsx:992-1006`, `en.ts:766-767`) | omission | Its own hairline-separated section below the list, off and disabled behind the `WIP` chip, ungated on the connection count exactly as upstream | coming soon — the pill ships in this change (#101). `registry.launchMode` and `registry.primary` are fields this app does not persist, so there is no stored value for the switch to report and a half-lit one would be a claim about a preference that is not saved. `ConnectionsJourneyTest`, `ConnectionRowActionsLayoutTest` |
| `saving ? s.saving : s.save` holds `Saving…` on the button while the write is in flight (`:951-952`, `en.ts:819`) | drift | No `Saving…` state | This editor closes on the press: the row write and the Keystore write run on the ViewModel's scope after the form is gone, so there is no window a label could say it in. Giving the button one means holding the editor open across the write, which changes what saving *does* rather than what it says; #85 |
| `urlTitle` placeholder is `http://homelab.lan:9119` (`connections-registry.tsx:803`) | mobile-adaptation | `https://homelab.lan:9119` | This app refuses a plain-HTTP Gateway URL, so upstream's scheme would demonstrate a value the form is about to reject. The vocabulary stays Desktop's: a placeholder teaches the *shape* of the answer, and "a machine on your LAN, on Hermes' port" is that shape where `hermes.example.com` was not (#85) |
| Desktop maps the `remote` kind to two different glyphs: `Globe` in the registry (`connections-registry.tsx:42-47`) and `Network` in the rail's `ConnectionGlyph` (`components/connection-glyph.tsx:19-26`) | mobile-adaptation | `Globe` on both surfaces | Viewport: upstream's two surfaces never meet — a statusbar and a settings page — so the disagreement costs it nothing. Here the rail and the registry are one screen apart, and a kind that changed shape between them would read as two kinds. The registry's mapping wins because the registry is where a kind is chosen and named (#85) |
...
| `signIn` composes the whole sign-in from the form's own `trimmedUrl`, in the window that form is in, and guards staleness with a sequence counter over its own awaits — never with the identity of a connection (`gateway-settings.tsx:607-659` @ `72a3277cd7`) | mobile-adaptation | The sign-in carries the row the tap was composed against; a stamp the store no longer calls active drops the tap, says so in one line, and opens no browser | Desktop's guard has nothing to be about: its sign-in target is text in the same component, so a sequence counter over its own steps is the whole risk. This pane is a *projection* of a saved row, and on a phone the switcher that moves the marker lives on another surface entirely — the sessions drawer — so the pane can be one store read behind it, and the target can change *identity* mid-tap. That window costs a character on the route form and a browser opened at the gateway the person just left here, which is why the sign-in writes under the same stamp rule the form does. `GatewaySettingsViewModelTest` drives the window, and asserts the same tap signs in once the pane has caught up |
| The desktop's own loopback listener catches the code in the process that opened the browser, so the flow returns into the window it left and there is nowhere else to land (`hermes_cli/dashboard_auth/native_flow.py:16-35`, step 4 @ `3ca096de5f8183cb2e0ec23673f294d5978656a3`) | mobile-adaptation | The hand-back Intent carries which surface the sign-in started from, and one started in the sessions drawer lands back on sessions rather than in Settings | Android hands the person to a *separate browser task* and back through a fresh Intent, which resumes whichever surface was last on screen. The owner's Scenario A is a drawer switch to a signed-out gateway: without this the person finishes signing in and is left one destination away from the sessions they were reaching for. `GatewaySignInBrowserTest` pins the value surviving the Intent, `SignInHandBackJourneyTest` the drawer marking the journey and the hand-back landing on sessions |
...
`review-desktop-parity` sets for that still applies.

Four states added by #116 are unrendered on the Android side and owed by that
issue: the switch-failure line at both switcher mounts, the current row's
`Sign in` / `Connect` action, the chat chrome's line as a tappable target, and
the row mid-switch that offers neither. They are stated here rather than as a
`- pending:` field because this page already names a report, and the gate
refuses a page that claims both (`check_visual_report`); the field would have
to replace the Desktop capture, which is real evidence. What the packet does give
...
node .chalk/skills/port-hermes-desktop-surface/scripts/capture-desktop-reference.mjs \
  ...
  --expect-sha 72a3277cd7937fd0f0a2a3e3fddbed21d7b1c8bd --match 5174
```

`contract.json` records `upstreamSha:
72a3277cd7937fd0f0a2a3e3fddbed21d7b1c8bd` and the root node's classes as
...
S-G4's second mount of the switcher (#110) adds no new Desktop surface to
capture — it is the statusbar control this packet's sibling
`connection-switcher.tsx` rows already cover, rendered somewhere else — so no
new reference was taken. What it does add is an Android-only *position*, which
only an Android capture could show, and this pane is `FLAG_SECURE`: it goes
with the device re-run #85 already carries. Until then the mount is covered by
`GatewayPaneSwitcherJourneyTest` rather than by eye.
