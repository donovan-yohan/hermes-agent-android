...
`3ca096de5f8183cb2e0ec23673f294d5978656a3`, read through
...
| `ConnectionSwitcher` mounted in the statusbar only (`app/shell/hooks/use-statusbar-items.tsx:411,617-621`) | mobile-adaptation | The same composable mounted a second time at the top of the Gateways pane, above the mode cards | Owner-approved 2026-08-30. Desktop's statusbar sits beside the settings window; a phone's Gateways screen is a destination you navigate to, so without this the person who just saved a gateway must leave the screen to use it. One composable, two mounts — trigger, sheet, order, checkmark, search threshold and select path unchanged. Desktop's hide-below-two rule (`connection-switcher.tsx:118-120`) applies on this pane as well |
...
| The statusbar is persistent chrome; its switcher never scrolls away (`app/shell/hooks/use-statusbar-items.tsx:411`) | mobile-adaptation | The Gateways mount is the first thing in the pane's scroll and scrolls with it | On screen when the screen opens, gone once you scroll to the registry. Pinning it would spend on a once-per-visit control the height #100 already declined to give a pinned mode grid; the session rail's mount is this app's always-reachable one |
...
node .chalk/skills/port-hermes-desktop-surface/scripts/capture-desktop-reference.mjs \
  ...
  --expect-sha 3ca096de5f8183cb2e0ec23673f294d5978656a3 --match 5174
```

`contract.json` records `upstreamSha:
3ca096de5f8183cb2e0ec23673f294d5978656a3` and the root node's classes as
