...
| What that match reads for a title | `apps/desktop/src/lib/chat-runtime.ts:51-53` (`sessionTitle`: title, else preview, else `Untitled session`) |
| Which family the leading glyph belongs to | `apps/desktop/src/lib/icons.ts:102` (`IconSearch as Search`, Tabler — only the clear button is a Codicon, `search-field.tsx:98`) |
...
| Leading glyph | Tabler `IconSearch`, `size-3.5`, muted (`search-field.tsx:69` renders `Search` from `@/lib/icons`, which is `IconSearch` at `lib/icons.ts:102`) | The Codicon `search`, `U+EA6D`, at 14 sp in `tokens.textTertiary`: the same shape from the family this app ships, the way `HermesIcon.Globe` and `HermesIcon.Monitor` already stand in for their lucide originals. Ledgered below |
...
| Instant client-side match | `sessionMatchesSearch` over the loaded rows (`session-search.ts:7-23`) | **Same** seven fields: id, lineage root, title, preview, cwd, git branch, source terms — with the title read raw rather than through Desktop's fallback, ledgered below |
| Source terms | id, label, aliases (`session-source.ts:121-130`) | **Same** table, **same** aliases, and the fallback uppercases at Desktop's own `\b\w` boundary (`session-source.ts:118`, `lib/text.ts:7`), not only after a space |
...
|---|---|---|---|
| The request carries `q` alone (`api/sessions.ts:348-352`) | mobile-adaptation | Sends `profile` while the rail stands in a named profile, through the same `scopeQuery` every other leg uses, and sends none in the unified view | Desktop's sidebar is one profile's by construction; this app's foot rail can stand in a named profile, and a search that ignored that scope would answer with conversations the list beside it does not show. The unified view is the case the route cannot serve — it opens exactly one profile's database (`sessions.py:227`) and has no `all` — so it sends no `profile` and the Gateway answers from its launch profile, which is the first leg `sessionListProfiles` already asks for. Sending the *active* profile there would merge one profile's server hits into an all-profile list |
...
| `⌘K` / `⌘P` command palette with a `Go to session` section (`app/command-palette/index.tsx`, `en.ts:1455` `paletteTitle`, `:1459` `goToSession`) | omission | Absent; the sidebar field is the whole session-lookup surface | non-goal: a phone has no chord keyboard to open it, and its session rows are a client-side scan of 200 loaded sessions — strictly less than the field's backend index. Same judgement as the System panel's palette row in `system-panel.md` |
...
| The `Results` merge reads `sessionTitle(session)` = `title \|\| preview \|\| 'Untitled session'` (`lib/chat-runtime.ts:51-53`, read by `session-search.ts:17`) | omission | The local match reads the raw `title`, so typing `untitled` finds nothing here and every nameless row on Desktop | deferred: #73 — this rail draws the raw title too (`SessionList.kt` `SessionRow`), so matching a row on a label it never shows would be a hit the reader cannot see. Rendering Desktop's fallback in the row is the change that makes the match field right, and it belongs with the row rather than with search |
| Desktop's shared `SearchField` always draws the leading glyph (`search-field.tsx:69`) | omission | The primitive takes it as an opt-in; only the sessions rail passes one, so the Gateways list, the profiles roster and the connection switcher keep the glyphless field they already render | deferred: #73 — adopting it at those three call sites restyles three surfaces this slice has no rendered capture for, which is the thing `docs/workflows/review-desktop-parity.md` refuses. The primitive is deliberately split until each of them is captured |
| Cross-profile search | omission | Search is scoped to one profile: the one the rail stands in, or the Gateway's launch profile while the rail is in the unified view | deferred: #73 — cross-profile search is its own bullet on that epic, and it needs a route this Gateway does not expose |
