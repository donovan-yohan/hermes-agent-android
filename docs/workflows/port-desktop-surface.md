...
|---|---|---|
| Theme registry | `apps/desktop/src/themes/presets.ts` (+ palettes in `apps/shared/src/theme-presets.ts`) | The built-in presets (eleven at the pin), labels, descriptions, default skin |
...
| Running-session row outline | `apps/desktop/src/store/session-dot-state.ts:70-80`; `apps/desktop/src/app/chat/sidebar/session-row.tsx:254-258,404`; `apps/desktop/src/styles.css:994-1008,1011-1040,1085-1113,1129-1144,1157-1161`; `apps/desktop/src/app/chat/sidebar/chrome.tsx:21-42,116-140` | Working and stalled alone own the flush 1.25px travelling outline; needs-input stays live but yields to its amber dot, and reduced motion leaves a static ring |
| Transcript tail and jump control | `apps/desktop/src/components/assistant-ui/thread/list.tsx:407-416,694-723`; `apps/desktop/src/app/chat/scroll-to-bottom-button.tsx` | Open at the newest content, follow only while parked there, and expose one floating return action after the reader scrolls away |
| Sticky current user prompt | `apps/desktop/src/components/assistant-ui/thread/list.tsx:194-232,350-372`; `apps/desktop/src/components/assistant-ui/thread/user-message.tsx:28-52,321-367`; `apps/desktop/src/styles.css:1538-1569`; `apps/desktop/src/components/assistant-ui/thread/timeline.tsx:113-125` @ `3ca096de5f8183cb2e0ec23673f294d5978656a3` | Group each human turn with the following assistant/tool content; pin that turn's bubble only after its source is fully above, and jump by stable message id without claiming scroll ownership |
...
| Sidebar section header | `apps/desktop/src/app/shell/sidebar-label.tsx`; `apps/desktop/src/app/chat/sidebar/sessions-section.tsx:62-100` | Uppercase accent label, tracking, dither mark, header rhythm and trailing action cluster |
| Sidebar header actions | `apps/desktop/src/app/chat/sidebar/index.tsx:246-256,1705-1786`; `apps/desktop/src/app/chat/sidebar/filter-menu.tsx:216-234` | Add-before-filter order, Codicon glyphs, 12px visual size, hover vs always-visible navigation treatment |
| Sidebar grouping selector | `apps/desktop/src/app/chat/sidebar/filter-menu.tsx:75-80,237-257`; `apps/desktop/src/store/layout.ts:217-294,359-362,589-620`; `layout-sidebar-view.test.ts:21-78` | "Updated" is the persisted default; "Project" switches the same catalog into the project tree and leaving it restores the time-grouped list |
| Project creation | `apps/desktop/src/app/chat/sidebar/project-dialog.tsx`; `apps/desktop/src/store/projects.ts:900-962` | New Project owns name + folder input and calls `projects.create`; the header plus is not New Session in overview mode |
...
| Remote lifecycle | `apps/desktop/electron/remote-lifecycle.ts`, `remote-lifecycle.test.ts`; `hermes_cli/main.py:538-546,692-717,11032-11106`; `hermes_cli/profiles.py:2540-2574` | Login-shell discovery, explicit default/named profile home resolution, OS-home-only token allowlist, exact ownership lock, exclusive/no-follow token upload, spawn-failure cleanup, and bounded TERM proof |
...
| Remote lock consumer | `hermes_cli/dashboard_procs.py:736-852` and `tests/hermes_cli/test_orphan_desktop_serve_reap.py:113-187` | The lock is a cross-runtime ABI: exact schema/field names/types/bounds/log suffix determine whether Hermes spares the live remote backend |
| Connection config | `apps/desktop/electron/connection-config.ts` | Remote profile and connection terminology |
| Saved connections registry | `apps/desktop/src/app/settings/connections-registry.tsx:26-31,89-168,221-888`; `settings/gateway-settings.tsx:1499-1502`; `settings/primitives.tsx:27-29,108-155`; `lib/connection-display.ts` | Kind glyphs, the duplicate rule, the `ListRow`/`Pill`/`EmptyState` grammar, and where the section sits on the page |
| Connection switcher and switch semantics | `apps/desktop/src/app/chat/sidebar/connection-switcher.tsx:40-322`; `src/store/connections.ts:153-225`; `src/store/gateway-switch.ts:47-96` | No chrome for one source, the eight-source search threshold, the radio group, and the exact wipe a source change performs. Full ledger: [`docs/parity/gateway-connections.md`](../parity/gateway-connections.md) |
...
node .chalk/skills/port-hermes-desktop-surface/scripts/capture-desktop-reference.mjs \
  ...
  --expect-sha 3ca096de5f8183cb2e0ec23673f294d5978656a3 \
  --match 5174
...
Two testing pitfalls came out of the same slice. A surface that polls while it
is visible has a loop with no end, so `advanceUntilIdle` spins the virtual clock
forever: drive it with explicit `advanceTimeBy(interval)` + `runCurrent()`
ticks. And that loop must be stopped *inside* the test body — `runTest` drains
the scheduler during its own teardown, so a live timed task there hangs the test
after its assertions have already passed, which reads like a mystery rather than
a leak.
