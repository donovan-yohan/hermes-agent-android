...
|---|---|---|---|
| App cold start, saved route | Boot connects; transient boot faults retry with backoff (max 5) | One-shot `restoreRemote`; failure = dead until manual tap | Same one-shot attempt, but failures enter the bounded retry loop instead of dying silently |
...
| Ticket/OAuth expired during outage | Refresh path; reauth surfaced **once** per episode, loop continues | Restore fails non-interactively ("Sign in to this Gateway before reconnecting") | Same message, but shown from the retry loop's honest-failure gate, not after 3 blind attempts |
...
3. **Attempt reset on stability.** A connection that held ≥30s counts as
   ...
   the ladder fresh.
4. **Wake/network nudges.** Online-transition, app foregrounding, and power
   resume trigger an *immediate* reconnect (attempt counter reset) rather
   than waiting out the current backoff sleep.
