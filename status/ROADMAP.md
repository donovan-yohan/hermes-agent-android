...
- The passive half is its own silent, ongoing group for the active connection:
  one summary notification — the same notification the foreground service holds
  — plus one child per live chat, taken from the Gateway's own
  `session.active_list` registry, so a chat Desktop, the TUI or another client
  started on that Gateway appears as well. A child carries the redacted session
  title, the project label when the backend knows one, and the last line or a
  truthful status; tapping one opens it. Opening a conversation does not retire
  its child, because the chat is still live. Two limits are stated rather than
  papered over: work owned by another process (a cron run, an inbound
  messaging-gateway turn) is not in that registry and never appears, and neither
  is work on a different saved connection. The emulator shade capture for this
  group is owed under #274.
...
- Rendered side-by-sides for the settings screen and for the shade at each kind
  ...
  against, which the parity page records rather than pretending otherwise. The
  activity group's shade is owed separately under #274.
...
| Android background lifecycle | Automatic Remote Gateway redials pause while the app is backgrounded and resume on foreground return. A `dataSync` foreground service (`TurnForegroundService`) and its persistent notification keep the process unfrozen — and the socket able to receive — while the Gateway reports live work (its own live-session registry, which includes chats another client started on the same Gateway process) or this app has a parked prompt or a turn on the wire, stopping after a 5-second linger grace once none of that is left and never holding longer than a 30-minute ceiling. Android may suspend or stop the app, so uninterrupted background connectivity is not claimed; socket retention across backgrounding and Doze on physical devices remains unproven on device/emulator hardware, and work owned by another process (a cron run, an inbound messaging-gateway turn) is neither reported by that registry nor inferred. | A justified Android lifecycle design with notification, power, privacy, reconnect, and process-death acceptance evidence, plus physical device validation for turn background retention. |
...
| Session management | Create, open, history, rename, delete, pin, archive/restore and read-state all go through authoritative Gateway routes. Branch, export, move-to-project and per-session colour render in Desktop's own menu slots but do nothing: each is disabled behind the `WIP` chip, which is what the menu owes a reader rather than a claim that the verb works. There is no bulk selection or archived-chats settings page. Session search rides `GET /api/sessions/search` with a 200 ms debounce and falls back to local matching on a Gateway that does not serve it; it is unproven against a live Gateway. | Authoritative Gateway methods and mobile journeys for every exposed action. |
...
| Notifications | Connected-only: notifications arrive while the app holds a live Gateway socket, and the socket holds for as long as the background-lifecycle row above says it does. The passive group projects the Gateway's own live-session registry into the shade — a silent ongoing summary plus one child per live chat, including chats another client started on that Gateway — while work owned by another process is outside that registry and is neither shown nor inferred. Once the socket is gone nothing arrives: there is no push infrastructure upstream. Prompts parked while the app was disconnected are replayed on reconnect, deferred during the post-connect quiet window, and raised once the window expires (unless already announced pre-disconnect or answered in-app, preserved across multiple outstanding prompts replayed one event at a time). The in-app surfaces still show it. `POST_NOTIFICATIONS` is requested once, at the first live Gateway; a refusal is respected rather than re-prompted, and re-enabling is an OS settings action. No physical-device pass has answered a real approval from a real shade. | A justified background-lifecycle decision with Doze and battery honesty, and the emulator passes: [#99](https://github.com/donovan-yohan/hermes-agent-android/issues/99) driving real events through the Termux Local route, and [#274](https://github.com/donovan-yohan/hermes-agent-android/issues/274) capturing the activity group's shade. |
