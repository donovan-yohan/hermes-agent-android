...
fun KanbanScreen(state: KanbanUiState, onBack: () -> Unit, onRefresh: () -> Unit, onOpenTask: (KanbanTask) -> Unit, onCloseDetail: () -> Unit) {
    ...
    OverlayScaffold(title = if (showingDetail) "Task" else "Kanban", backDescription = if (showingDetail) "Back to board" else "Back", onBack = { if (showingDetail) onCloseDetail() else onBack() }) {
        TextButton(label = "Refresh", onClick = onRefresh, modifier = Modifier.padding(horizontal = HermesTheme.spacing.pageInset).testTag("Kanban refresh"))
        if (!showingDetail) {
            ...
            is KanbanDetail.Value -> { if (detail.stale) Notice(KANBAN_DETAIL_STALE); TaskDetail(detail.detail) }
            KanbanDetail.None -> Unit
        }
    }
}

@Composable private fun Notice(message: String) = Text(message, style = HermesTheme.type.caption, color = HermesTheme.tokens.textTertiary, modifier = Modifier.padding(HermesTheme.spacing.pageInset))
...
@Composable private fun TaskDetail(detail: KanbanTaskDetail) = SelectionContainer { Column(Modifier.fillMaxWidth().padding(HermesTheme.spacing.pageInset).testTag("Kanban detail"), verticalArrangement = Arrangement.spacedBy(HermesTheme.spacing.turnGap)) {
    ...
    task.assignee?.let { Field("Assignee", it) }; task.priority?.let { Field("Priority", it.toString()) }
    task.body?.let { Field("Description", it) }; task.latestSummary?.let { Field("Latest summary", it) }; task.result?.let { Field("Result", it) }
    ...
    if (detail.childResults.isNotEmpty()) Field("Child results", detail.childResults.joinToString("\n") { "${it.id} · ${it.status}" })
    Text("Read-only task snapshot.", style = HermesTheme.type.caption, color = HermesTheme.tokens.textTertiary)
} }
@Composable private fun Field(label: String, value: String) = Column(verticalArrangement = Arrangement.spacedBy(HermesTheme.spacing.turnGap)) {
