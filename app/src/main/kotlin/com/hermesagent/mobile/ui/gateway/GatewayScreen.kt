 ...
 * gateway, Connect via SSH (`gateway-settings.tsx:1049-1082` @ `936b970e`).
 *
...
fun GatewayScreen(
    ...
    // for the chosen mode (`gateway-settings.tsx:1044-1089` @ `936b970e`).
    ...
    val chooser: @Composable ColumnScope.() -> Unit = {
        ...
        // `936b970e`); this second mount is the owner-approved mobile
    ...
    Column(modifier.fillMaxSize().background(tokens.chatSurface)) {
        Box(Modifier.weight(1f)) {
            ...
            // `936b970e`). On a phone the page is one scroll per route, so the
            ...
            when (state.mode) {
                GatewayConnectionMode.Remote -> RemoteGatewayScreen(
                    ...
                    // (`connections-registry.tsx:120-168` @ `936b970e`). This
